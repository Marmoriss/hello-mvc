...
-- select * from (select row_number () over (order by reg_date desc) rnum, b.* from board b) b where rnum between 1 and 5;
...
delete from board_comment where no = 8;
-- 121번 게시글 테스트댓글
insert into board_comment
values(seq_board_comment_no.nextval, default, 'abcde', '꽃이 참 예뻐요~!', 121, null, default);

insert into board_comment
values(seq_board_comment_no.nextval, default, 'admin', '이 달의 best게시글로 선정되었습니다.', 121, null, default);

insert into board_comment
values(seq_board_comment_no.nextval, default, 'sinsa', '꽃 한 송이 놓고 갑니다 @------%--', 121, null, default);

-- 121번 게시글 테스트 대댓글
insert into board_comment
values(seq_board_comment_no.nextval, 2, 'honggd', '정말 예쁘죠 ^^*', 121, 1, default);

insert into board_comment
values(seq_board_comment_no.nextval, 2, 'qwerty', '와 작성자님 축하드려요~!~!', 121, 2, default);

insert into board_comment
values(seq_board_comment_no.nextval, 2, 'qwerty', '저도 한 송이 놓고갑니다 총총 @---&--', 121, 3, default);

--insert into board_comment values(seq_board_comment_no.nextval, ?, ?, ?, ?, ?, default)

-- 계층형 쿼리
-- 부모레코드와 자식 레코드를 연결해서 조회하는 쿼리
-- 댓글트리, 메뉴, 조직도, 가계도 표현에 적합함.
-- start with : 최상위 record 조건절(여러개일 수 있다)
-- connect by 부모레코드(prior키워드)와 자식레코드의 관계 설정
...
--select *from board_comment where board_no = ? start with comment_level = 1 connect by prior no = comment_ref order siblings by no desc
    
-- kh.employee 조직도
select * from employee;

select
    level, -- 계층형 쿼리에서만 사용 가능
    emp_id, 
...
select * from ( select row_number() over(order by no desc) rnum, b.*, (select count(*) from attachment where board_no = b.no) attach_count, (select count(*) from board_comment where board_no = b.no) comment_count from board b) b where rnum between 1 and 5;
-- 댓글 개수 포함 게시글 목록 조회
select 
    (select count(*) 
    from board_comment
